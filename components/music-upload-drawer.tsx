    ...
    <div>
          ...
          {!musicFile ? (
            <button
              ...
              className={`w-full rounded-lg border-2 border-dashed p-8 text-center transition-colors ${
                isDragging ? "border-yellow-400 bg-yellow-400/5" : "border-border bg-muted/30"
              }`}
            >
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-4">
              <div className="flex items-center gap-3">
                <div className="flex size-12 items-center justify-center rounded-lg bg-yellow-400/20">
      ...
      <div className="mt-7 flex gap-4">
        <FamilyDrawerSecondaryButton
          ...
          disabled={!musicFile}
          className={`bg-yellow-400 text-black ${!musicFile ? "opacity-50 cursor-not-allowed" : ""}`}
        >
    ...
    <div>
        ...
        <div className="mt-6">
          {!coverImage ? (
            ...
            <div className="relative">
              <div className="aspect-square w-full max-w-[200px] mx-auto rounded-lg overflow-hidden border border-border">
              ...
              <p className="text-center mt-3 text-xs text-muted-foreground">{coverImage.file.name}</p>
            </div>
          )}
        </div>
      </div>
      <div className="mt-7 flex gap-4">
        ...
        <FamilyDrawerSecondaryButton
          ...
          // @ts-expect-error - disabled prop is not defined in FamilyDrawerSecondaryButton
          disabled={!coverImage}
          className={`bg-yellow-400 text-black ${!coverImage ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          Continue <ArrowRight className="size-4" />
        </FamilyDrawerSecondaryButton>
      </div>
            ...
            <div>
              ...
              <div className="mt-1 relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full rounded-lg border border-border bg-background pl-7 pr-3 py-1.5 text-sm"
                />
              </div>
            </div>
          </div>
...
function ConfirmView() {
  const { setView } = useFamilyDrawer()
  const { musicFile, coverImage, metadata } = uploadState
  ...
  return (
    <div>
      ...
      <div className="mt-7 flex gap-4">
        <FamilyDrawerSecondaryButton onClick={() => setView("metadata")} className="bg-secondary text-secondary-foreground">
          <ArrowLeft className="size-4" /> Back
        </FamilyDrawerSecondaryButton>
        <FamilyDrawerSecondaryButton onClick={() => setView("uploading")} className="bg-yellow-400 text-black">
...
function UploadingView() {
  ...
  return (
    <div>
      <div className="px-2">
        <FamilyDrawerHeader
          icon={<Upload className="size-12 text-yellow-400 animate-pulse" />}
          title="Uploading..."
          description="Please wait while we upload your track"
        />
        <div className="mt-6 space-y-4 border-t border-border pt-6">
          ...
          <div className="space-y-2 text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              {progress >= 30 ? <CheckCircle className="size-4 text-yellow-400" /> : <div className="size-4 rounded-full border-2 border-muted-foreground border-t-transparent animate-spin" />}
              <span>Uploading audio file...</span>
            </div>
            <div className="flex items-center gap-2">
              {progress >= 60 ? <CheckCircle className="size-4 text-yellow-400" /> : progress >= 30 ? <div className="size-4 rounded-full border-2 border-muted-foreground border-t-transparent animate-spin" /> : <div className="size-4" />}
              <span>Uploading cover image...</span>
            </div>
            <div className="flex items-center gap-2">
              {progress >= 100 ? <CheckCircle className="size-4 text-yellow-400" /> : progress >= 60 ? <div className="size-4 rounded-full border-2 border-muted-foreground border-t-transparent animate-spin" /> : <div className="size-4" />}
